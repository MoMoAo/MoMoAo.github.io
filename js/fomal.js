...
function tonav() {
  ...
  document.getElementById("page-name").innerText = document.title.split(" | MoMoAo🍓")[0];
}
...
function showWelcome() {
  ...
  let dist = getDistance(43.3211, 125.3947, ipLoacation.result.location.lng, ipLoacation.result.location.lat); //这里换成自己的经纬度
    ...
    case "中国":
      ...
      switch (ipLoacation.result.ad_info.province) {
        ...
        case "天津市":
          ...
          break;
        case "河北省":
          posdesc = "山势巍巍成壁垒，天下雄关。铁马金戈由此向，无限江山。我也河北滴~";
          break;
        case "山西省":
...
function createtime1() {
  var grt = new Date("08/25/2023 00:00:00"); //此处修改你的建站时间或者网站上线时间
...
function createtime2() {
  ...
  setTimeout(
    console.warn.bind(
      console,
      "%c ⚡ Powered by MoMoAo🍓 %c 你正在访问MoMoAo🍓の小家",
...
function share_() {
  ...
  try {
    ...
    var subTitle = title.endsWith("| MoMoAo🍓") ? title.substring(0, title.length - 14) : title;
    navigator.clipboard.writeText('MoMoAo🍓的站内分享\n标题：' + subTitle + '\n链接：' + url + '\n欢迎来访!🍓🍓🍓');
    new Vue({
      data: function () {
        this.$notify({
          title: "成功复制分享信息🎉",
...
document.addEventListener('visibilitychange', function () {
  if (document.hidden) {
    //离开当前页面时标签显示内容
    document.title = '🏃‍♂️跑哪去了！';
    clearTimeout(titleTime);
  } else {
    //返回当前页面时标签显示内容
    document.title = '🧍‍♂️找到你啦~';
    //两秒后变回正常标题
...
if (localStorage.getItem("blogbg") != undefined) {
  ...
  document.getElementById("defineBg").innerText = `:root{
    --default-bg: url(https://momoao.github.io/assets/day.jpg);
    --darkmode-bg:url(https://momoao.github.io/assets/night.jpg);
    --mobileday-bg: url(https://momoao.github.io/assets/mobileday.jpg);
    --mobilenight-bg: url(https://momoao.github.io/assets/mobilenight.jpg);
  }`;
}
// 切换背景主函数
function changeBg(s) {
